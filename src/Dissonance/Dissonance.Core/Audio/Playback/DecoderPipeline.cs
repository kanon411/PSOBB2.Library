...
namespace Dissonance.Audio.Playback
{
		...
		: IDecoderPipeline, IVolumeProvider
	{
		...
		public float Push(VoicePacket packet, DateTime now, bool isAudioDataShared = true)
		{
			Log.Trace("Received frame {0} from network", packet.SequenceNumber);

			//This allows us to avoid a second copy in GladNet cases where the audio data may not be shared, precopied.
			var packetCopy = isAudioDataShared
				? new VoicePacket(packet.SenderPlayerId,
					packet.PlaybackOptions.AmplitudeMultiplier,
					packet.PlaybackOptions.IsPositional,
					packet.EncodedAudioFrame.CopyTo(_bytePool.Get()),
