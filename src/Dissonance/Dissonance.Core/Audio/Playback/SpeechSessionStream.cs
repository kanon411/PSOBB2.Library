...
namespace Dissonance.Audio.Playback
{
		...
		: IJitterEstimator
	{
		...
		public void StartSession(FrameFormat format, DateTime? now = null, [CanBeNull] IJitterEstimator jitter = null)
		{
			...
			var session = SpeechSession.Create(new SessionContext(PlayerName, unchecked(_currentId++)), jitter ?? this, _active, now ?? DateTime.UtcNow);
		...
		public void ReceiveFrame(VoicePacket packet, DateTime? now = null)
		{
			...
			//TODO: We're claiming the audio isn't shared, if it ever it we will have problems.
			CustomProjectDissonanceAsserts.AssertVoicePacketAudioDataAlreadyCopied();
			var delay = _active.Push(packet, now ?? DateTime.UtcNow, false);
