    ...
    internal class EventQueue
    {
        ...
        private void PreDispatchLog(DateTime utcNow)
        {
            ...
            if (delta > MinWarnDispatchTimeThreshold)
                ...
                Log.Warn("Large number of received packets pending dispatch ({0}). Possibly due to network congension (last frame was {1}ms)", _pendingVoicePackets, delta.TotalMilliseconds);
        }
