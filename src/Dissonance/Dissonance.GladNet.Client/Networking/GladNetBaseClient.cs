...
{
	public abstract class GladNetBaseClient<TClient, TPeer>
		: IClient<TPeer>
		where TPeer : struct
		where TClient : GladNetBaseClient<TClient, TPeer>
	{
		...
		protected readonly EventQueue _events;
		...
		#region constructors
		protected GladNetBaseClient([NotNull] ICommsNetworkState network)
		{
		...
		private ushort? ProcessReceivedPacket(ArraySegment<byte> data)
		{
			...
			switch(header)
			{
				...
				case MessageTypes.TextData:
					if(CheckSessionId(ref reader, header))
					{
						throw new NotSupportedException("Encountered TEXT MESSAGE from Dissonance. We should strip these packets on the server side.");
				...
				case MessageTypes.RemoveClient:
					throw new NotSupportedException($"Encountered {MessageTypes.RemoveClient} from Dissonance. We should strip these packets on the server side.");
					if(CheckSessionId(ref reader, header))
					{
				...
				case MessageTypes.ClientState:
					//throw new NotSupportedException($"Encountered {MessageTypes.ClientState} from Dissonance. We should strip these packets on the server side.");
					if(CheckSessionId(ref reader, header))
					{
		...
		void IClient<TPeer>.SendReliableP2P(List<ClientInfo<TPeer?>> destinations, ArraySegment<byte> packet)
		{
			UnityEngine.Debug.LogError($"Encountered P2P packet: {(MessageTypes)packet.Array[packet.Offset + 2]}. Should trim this, not forwarding.");
		}

		void IClient<TPeer>.SendUnreliableP2P(List<ClientInfo<TPeer?>> destinations, ArraySegment<byte> packet)
		{
			UnityEngine.Debug.LogError($"Encountered P2P packet: {(MessageTypes)packet.Array[packet.Offset + 2]}. Should trim this, not forwarding.");
		}
		#endregion
